...
# Convert PIL image to base64 string for displaying in browser
# (Streamlit uses base64 to render images inside HTML)
def img_to_base64(image : Image.Image) -> str:
    buffered = io.BytesIO()
    image.save(buffered, format="PNG")
    return base64.b64encode(buffered.getvalue()).decode()

# Initialize session variables to store metadata and UI state
def init_session_state():
    session_defaults = {
    "metadata" : None, # Stores all processed image data
    "unique_classes" : [], # Stores all unique detected classes
    "count_options" : {}, # Stores available count ranges per class
    "search_results" : [], # Stores filtered images after search
    "search_params" : {
...
# Option to process new images or load already processed metadata
# Main options
...
if st.session_state.search_results:
    ...
    for result in results:
        with grid_cols[col_index]:
            try:
                img = Image.open(result["image_path"])# Load original image
